...
def test_customer_service_has_tenant_and_version() -> None:
    ...
    assert instance.display_name == "常州太平洋口腔语音客服"
    assert instance.organization_name == "常州太平洋口腔"
    assert "太平洋口腔" in instance.tenant_prompt
    assert "400-0519-020" in instance.tenant_prompt
    assert "症状沟通" in instance.platform_prompt
    assert "种植牙" in instance.platform_prompt
    assert "项目怎么做" in instance.platform_prompt
    assert "少用「需要医生检查后才能确定」" in instance.platform_prompt
    assert "400-0519-020" in instance.tenant_prompt
    assert "局前街" in instance.tenant_prompt
...
def test_demo_greeting_avoids_identity_disclosure_claims() -> None:
    ...
    assert "常州太平洋口腔客服" in instance.greeting
    assert "AI" not in instance.greeting
    assert "人工" not in instance.greeting
