...
> **持久化**：未设置 `DATABASE_URL` 时，客服配置与通话记录仍在进程内存中（重启丢失）。
> 设置 `DATABASE_URL` 并完成 Alembic 迁移后，使用本机 Docker PostgreSQL 持久化。
> 本模块不是电话 CDR，也不包含生产认证或全局运营端 RBAC。
...
可选：启用 PostgreSQL（仓库根目录 Docker Compose）：
...
也可把同一 `DATABASE_URL` 写入 `.env.local`（勿提交）。
