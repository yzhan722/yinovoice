...
def published_snapshot(
    *,
    ...
    return {
        ...
        "platform_prompt": "Platform dialogue framework.",
...
async def test_client_fetches_exact_tenant_snapshot() -> None:
    ...
    assert result.voice.tts_voice == "longanqian"
    assert result.platform_prompt == "Platform dialogue framework."
    assert result.tenant_prompt == "Internal platform-only prompt."


@pytest.mark.asyncio
async def test_client_accepts_snapshot_with_null_deleted_at() -> None:
    service_id = uuid4()
    tenant_id = uuid4()
...
async def test_client_rejects_soft_deleted_snapshot() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        snapshot["deleted_at"] = "2026-08-17T01:00:00Z"
        return httpx.Response(200, json=snapshot)
    ...
    async with httpx.AsyncClient(
        ...
        with pytest.raises(RuntimeConfigurationError, match="soft-deleted"):
